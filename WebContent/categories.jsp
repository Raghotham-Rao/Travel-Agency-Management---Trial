...
<html>
	...
	<body>
		...
		<div id = "header">
			<% if(request.getParameter("cat")=="Holidayspot")
				out.println("Holiday-Spots");
			   else if(request.getParameter("cat")=="Onedaytours")
				out.println("One-Day-Tours");
			   else
		...
		<%try {
			...
			String search = "select * from packagetours where category='"+request.getParameter("cat")+"'";
			...
			while(rs.next()) {
				...
				while(itr.hasNext())
					tag += "<li>"+itr.next()+"</li>";
				tag += "</ul><p class = \"detailsbutton\"><form action=\"tripdetails.jsp\" method=\"post\"><input type = \"hidden\" value = \""+rs.getString(1)+"\" name=\"tourid\"/><input type = \"submit\" value = \"View Details\"/></form></p></div>";
